...
function statusLabel(status: string) { return status === "on_site" ? "covered" : status.replaceAll("_", " ") }
...
export default function EventLogistics({ eventId, eventStatus, api = logisticsApi }: { eventId: number; eventStatus: "open" | "closed"; api?: LogisticsApi }) {
  ...
  async function cancelRequirement(requirement: LogisticsRequirement) {
    if (isClosed || !window.confirm(`Cancel the Requirement “${requirement.name}”? Its history will be preserved.`)) return
    setSaving(true); setFeedback("")
    try { await api.cancelEventRequirement(eventId, requirement.id); setSelected(null); await loadModel(); setFeedback("Requirement cancelled.") } catch (cancelError) { setFeedback(message(cancelError)) } finally { setSaving(false) }
  }

  async function reserve(requirement: LogisticsRequirement) {
    setSaving(true); setFeedback("")
    try { await api.reserveInventory(eventId, requirement.id, { location_id: Number(form.location_id), quantity: Number(form.reserve_quantity) }); setSelected(null); await loadModel(); setFeedback("Inventory reserved.") } catch (saveError) { setFeedback(message(saveError)) } finally { setSaving(false) }
  }
  async function allocationAction(requirement: LogisticsRequirement, allocationId: number, action: "release" | "issue") {
    setSaving(true); setFeedback("")
    try { await api.allocationAction(eventId, requirement.id, allocationId, action, { quantity: Number(form[`allocation_${allocationId}`] || 0) }); setSelected(null); await loadModel() } catch (saveError) { setFeedback(message(saveError)) } finally { setSaving(false) }
  }
  async function backfill(requirement: LogisticsRequirement) {
    setSaving(true); setFeedback("")
    try { await api.backfillRequirement(eventId, requirement.id, { quantity: Number(form.backfill_quantity), notes: form.backfill_notes || "" }); setSelected(null); await loadModel() } catch (saveError) { setFeedback(message(saveError)) } finally { setSaving(false) }
  }
  async function allocateRemaining(requirement: LogisticsRequirement) {
    const quantityToBackfill = Math.max(requirement.not_yet_on_site, 0)
    if (isClosed || quantityToBackfill <= 0) return
    setSaving(true); setFeedback("")
    try {
      await api.backfillRequirement(eventId, requirement.id, { quantity: quantityToBackfill, notes: "Marked covered manually" })
      setSelected(null); await loadModel(); setFeedback(`${requirement.name} allocation saved.`)
    } catch (saveError) { setFeedback(message(saveError)) } finally { setSaving(false) }
  }
  ...
  async function sourceExternally(requirement: LogisticsRequirement) {
    ...
    try {
      ...
      setSelected(null); await loadModel(); setFeedback(`Supplier Order #${order.id} confirmed.`)
    } catch (saveError) { setFeedback(message(saveError)) } finally { setSaving(false) }
  }
  ...
  return <section className="event-logistics-workspace">
    ...
    <div className="inventory-table-scroll"><table className="event-logistics-table"><thead><tr><th>Requirement</th><th>Type</th><th>Required</th><th>Inventory Reserved</th><th>Supplier Ordered</th><th>On Site</th><th>Still to Source</th><th>Not Yet on Site</th><th>Needed By</th><th>Status</th><th>Action</th></tr></thead><tbody>{model.requirements.map((requirement) => <tr key={requirement.id}><td><button aria-label={`Open ${requirement.name} logistics`} className="requirement-link" onClick={() => openRequirement(requirement)}><strong>{requirement.name}</strong><small>{requirement.priority} priority</small></button></td><td>{requirement.requirement_type}</td><td>{quantity(requirement.required_quantity, requirement.unit)}</td><td>{quantity(requirement.inventory_reserved, requirement.unit)}</td><td>{quantity(requirement.supplier_ordered, requirement.unit)}</td><td>{quantity(requirement.on_site, requirement.unit)}</td><td className={requirement.still_to_source > 0 ? "logistics-short" : ""}>{quantity(requirement.still_to_source, requirement.unit)}</td><td className={requirement.not_yet_on_site > 0 ? "logistics-late" : ""}>{quantity(requirement.not_yet_on_site, requirement.unit)}</td><td>{formatDate(requirement.needed_by)}</td><td><span className={`inventory-chip logistics-${requirement.status}`}>{statusLabel(requirement.status)}</span></td><td>{!isClosed && requirement.status !== "fulfilled" && requirement.status !== "cancelled" && <button type="button" onClick={() => openRequirement(requirement)}>Allocate</button>}</td></tr>)}</tbody></table>{!model.requirements.length && <p className="inventory-empty">No Logistics Requirements yet.</p>}</div>
    ...
    {selected && <div className="logistics-drawer-backdrop" onClick={() => setSelected(null)}><aside aria-label={selected === "new" ? "New Requirement" : `${selected.name} logistics`} aria-modal="true" className="logistics-drawer" role="dialog" onClick={(event) => event.stopPropagation()}><header><div><p>{selected === "new" ? "Requirement planning" : selected.requirement_type}</p><h2>{selected === "new" ? "Add Requirement" : selected.name}</h2></div><button aria-label="Close logistics drawer" onClick={() => setSelected(null)}>×</button></header><div className="logistics-drawer-body">{selected === "new" ? <NewRequirementFields form={form} field={field} items={items} /> : <RequirementDetails requirement={selected} form={form} field={field} locations={locations} organizations={organizations} isClosed={isClosed} saving={saving} onReserve={() => void reserve(selected)} onBackfill={() => void backfill(selected)} onMarkCovered={() => void allocateRemaining(selected)} onAllocation={allocationAction} onReconcile={reconcile} onSource={() => void sourceExternally(selected)} reconciliationNotes={reconciliationNotes} setReconciliationNotes={setReconciliationNotes} />}{feedback && <p className="inventory-feedback">{feedback}</p>}</div><footer><button onClick={() => setSelected(null)}>Close</button>{selected !== "new" && !isClosed && <button className="text-danger" disabled={saving} onClick={() => void cancelRequirement(selected)}>Cancel Requirement</button>}{!isClosed && <button disabled={saving} onClick={() => void saveRequirement()}>{saving ? "Saving…" : "Save Requirement"}</button>}</footer></aside></div>}
  </section>
}
...
function RequirementDetails({ requirement, form, field, locations, organizations, isClosed, saving, onReserve, onBackfill, onMarkCovered, onAllocation, onReconcile, onSource, reconciliationNotes, setReconciliationNotes }: { requirement: LogisticsRequirement; form: Record<string, string>; field: (name: string, value: string) => void; locations: InventoryLocation[]; organizations: ExternalOrganization[]; isClosed: boolean; saving: boolean; onReserve: () => void; onBackfill: () => void; onMarkCovered: () => void; onAllocation: (requirement: LogisticsRequirement, allocationId: number, action: "release" | "issue") => void; onReconcile: (requirement: LogisticsRequirement, allocationId: number, issued: number) => void; onSource: () => void; reconciliationNotes: string; setReconciliationNotes: (value: string) => void }) { return <>
  ...
  <section><h3>Allocation & sourcing</h3><dl className="logistics-summary-grid"><div><dt>Inventory coverage</dt><dd>{quantity(requirement.inventory_reserved + requirement.inventory_issued, requirement.unit)}</dd></div><div><dt>External coverage</dt><dd>{quantity(requirement.supplier_ordered, requirement.unit)}</dd></div><div><dt>On site</dt><dd>{quantity(requirement.on_site, requirement.unit)}</dd></div><div><dt>Shortage</dt><dd>{quantity(requirement.still_to_source, requirement.unit)}</dd></div></dl>
    ...
    {!isClosed && <div className="logistics-backfill"><h4>Already on site?</h4><p>Record a quantity that was already provided when no allocation or supplier log exists.</p><div className="logistics-inline-form"><input aria-label="Backfill on-site quantity" min="0" step="0.001" type="number" value={form.backfill_quantity || ""} onChange={(event) => field("backfill_quantity", event.target.value)} placeholder={requirement.unit} /><input aria-label="Backfill notes" value={form.backfill_notes || ""} onChange={(event) => field("backfill_notes", event.target.value)} placeholder="Reason or source" /><button disabled={saving || !form.backfill_quantity} onClick={onBackfill}>Backfill on site</button></div>{requirement.not_yet_on_site > 0 && <button type="button" disabled={saving} onClick={onMarkCovered}>Allocate remaining ({quantity(requirement.not_yet_on_site, requirement.unit)})</button>}</div>}
  </section>
